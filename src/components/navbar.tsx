...
const Navbar = () => {
    ...
    return (
        <nav className='w-[95%] max-w-7xl px-5 md:px-10 py-4 backdrop-blur-lg shadow-md absolute top-2 md:top-5 left-1/2 -translate-x-1/2 z-10 rounded-md flex items-center justify-between'>
            ...
            <nav className={`w-full h-fit backdrop-blur-3xl bg-white rounded-sm shadow-lg flex flex-col items-start absolute top-16 p-5 space-y-5 md:hidden ${isOpen ? "left-1/2 -translate-x-1/2" : "-left-full"} transition-all duration-300 ease-in`}>
                {
                    navlinks.map(
                        (link, idx) => <Link
                            href={link.path}
